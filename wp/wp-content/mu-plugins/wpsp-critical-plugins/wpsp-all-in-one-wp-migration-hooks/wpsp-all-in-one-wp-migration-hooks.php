<?php
/**
 * @package wpsp-all-in-one-wp-migration-hooks
 */
/*
Plugin Name: wpsp-all-in-one-wp-migration-hooks
...
add_action( 'ai1wm_status_import_done', function($params) {
  ...
  $apiUrl = 'https://ctrl.'.WPSP_PLATFORM_DOMAIN.'/api-v1/site/update/';
  ...
  $dataToSend = [
      'user'       => WPSP_SITE_ID,
      'key'        => AUTH_KEY,
      'site-id'    => WPSP_SITE_ID,
      'task-name'  => 'run_wpcli_cmd',
      'wp-cli-cmd' => 'wp core update-db --skip-packages --skip-plugins --skip-themes --url='.WPSP_CURRENT_DOMAIN.' && 
                       wp media regenerate --yes --skip-packages --skip-plugins --skip-themes --url='.WPSP_CURRENT_DOMAIN.' && 
                       find /mnt/network-share-lower/network-share/wp-content/site'.WPSP_SITE_ID.'/uploads/ -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o -iname "*.bmp" -o -iname "*.tiff" -o -iname "*.tif" -o -iname "*.webp" -o -iname "*.svg" -o -iname "*.ico" -o -iname "*.heif" -o -iname "*.heic" -o -iname "*.j2k" -o -iname "*.jp2" -o -iname "*.jpf" -o -iname "*.jpx" -o -iname "*.jpm" -o -iname "*.avif" -o -iname "*.raw" -o -iname "*.cr2" -o -iname "*.nef" -o -iname "*.orf" -o -iname "*.sr2" \) -delete &&
                       find /mnt/network-share/wp-content/site'.WPSP_SITE_ID.'/uploads/ -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o -iname "*.bmp" -o -iname "*.tiff" -o -iname "*.tif" -o -iname "*.webp" -o -iname "*.svg" -o -iname "*.ico" -o -iname "*.heif" -o -iname "*.heic" -o -iname "*.j2k" -o -iname "*.jp2" -o -iname "*.jpf" -o -iname "*.jpx" -o -iname "*.jpm" -o -iname "*.avif" -o -iname "*.raw" -o -iname "*.cr2" -o -iname "*.nef" -o -iname "*.orf" -o -iname "*.sr2" \) -delete &&
...
if( defined( 'REQUIRED_FILE_all_in_one_wp_migration_s3_client_extension' ) &&  REQUIRED_FILE_all_in_one_wp_migration_s3_client_extension ){
  ...
  add_action( 'init', function(){
    ...
    ###
    if ( !wp_next_scheduled('wpsp_take_ai1wm_backup_on_schedule') ){
      wp_schedule_event( time(), 'twicedaily', 'wpsp_take_ai1wm_backup_on_schedule' );
    }
  ...
  add_action( 'wpsp_take_ai1wm_backup_on_schedule', function(){
    wpsp_take_ai1wm_backup_now();
  });
  ...
  add_filter( 'pre_option_ai1wmne_s3_api_endpoint', function($default){ return WPSP_CLOUDFLARE_R2_ACCOUNT_ID.'.r2.cloudflarestorage.com'; });
  add_filter( 'pre_option_ai1wmne_s3_bucket_template', function($default){ return WPSP_CLOUDFLARE_R2_BUCKET.'.'.WPSP_CLOUDFLARE_R2_ACCOUNT_ID.'.r2.cloudflarestorage.com'; });
  add_filter( 'pre_option_ai1wmne_s3_region_name', function($default){ return 'auto'; });
  add_filter( 'pre_option_ai1wmne_s3_access_key', function($default){ return WPSP_CLOUDFLARE_R2_API_KEY; });
  add_filter( 'pre_option_ai1wmne_s3_secret_key', function($default){ return WPSP_CLOUDFLARE_R2_API_VALUE_S3; });
  add_filter( 'pre_option_ai1wmne_s3_https_protocol', function($default){ return '1'; });

  add_filter( 'pre_option_ai1wmne_s3_bucket_name', function($default){ return WPSP_CLOUDFLARE_R2_BUCKET; });
  add_filter( 'pre_option_ai1wmne_s3_folder_name', function($default){ return WPSP_SITE_ID.'-wpspos/_ai1wm-backups'; });
  ...
  function wpsp_take_ai1wm_backup_now(){
